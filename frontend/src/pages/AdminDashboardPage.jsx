...
function ParticipantModal({ onClose, onDelete, participant }) {
  ...
  useEffect(() => {
    if (participant.id && !docsLoaded) {
      apiRequest(`/admin/registrations/${participant.id}/documents`).then(setDocs).catch(() => {}).finally(() => setDocsLoaded(true));
    }
  }, [participant.id, docsLoaded]);
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 p-5" onClick={onClose}><section className="content-transition w-full max-w-lg rounded-lg bg-white p-6 shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(event) => event.stopPropagation()}><div className="flex items-start justify-between"><div className="flex items-center gap-4"><div className="grid h-20 w-20 place-items-center overflow-hidden rounded-full bg-teal-50 font-extrabold text-teal-700">{participant.photo ? <img src={participant.photo} alt={participant.name} className="h-full w-full object-cover" /> : initials(participant.name)}</div><div><h2 className="text-xl font-extrabold">{participant.name}</h2><p className="mt-1 text-xs text-slate-400">Profil peserta BESC</p></div></div><button type="button" onClick={onClose} className="text-xl text-slate-400">×</button></div><div className="mt-6 grid gap-3 sm:grid-cols-2">{fields.map(([label, value]) => <Setting key={label} label={label} value={value} />)}</div>{docs.length > 0 && <div className="mt-6"><h3 className="mb-3 text-sm font-bold text-slate-800">Dokumen Upload</h3><div className="space-y-2">{docs.map((doc) => <a key={doc.id} href={`${API_URL}/admin/documents/${doc.id}/view`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 rounded-lg border border-slate-200 p-3 text-sm hover:bg-slate-50"><span className="text-lg">📄</span><div><div className="font-semibold text-slate-700">{doc.doc_type.replace(/_/g, ' ')}</div><div className="text-xs text-slate-400">{doc.original_name}</div></div></a>)}</div></div>}<div className="mt-6 flex justify-end gap-3"><button type="button" onClick={onClose} className="rounded-lg border border-slate-200 px-4 py-2 text-xs font-extrabold">Tutup</button><button type="button" onClick={onDelete} className="rounded-lg bg-red-600 px-4 py-2 text-xs font-extrabold text-white">Hapus Peserta</button></div></section></div>;
}
function ParticipantModal({ onClose, onDelete, participant }) {
  const [docs, setDocs] = useState([]);
  const [docsLoaded, setDocsLoaded] = useState(false);
